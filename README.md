...
| `Sources/cupertino-symbolgraphs-audit/` | Diagnostic CLI (ArgumentParser); subcommands for `validate-corpus`, `framework-stats`, `count-by-status`. Replaces ad-hoc Python heredocs with a Swift binary that ships in the package, has --help, and is unit-tested. |
